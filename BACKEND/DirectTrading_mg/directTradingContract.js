...
class DirectContractsContract extends Contract {
    ...
    /**
     * Getting direct contract for specific contract ID.
     *
     * @param {Context} ctx the transaction context
     * @param {String} ID the ID of contract
     *
     * @returns {JSON} the query results
     * @throws {shim.error} transaction execution failure
     */
    async getContractById(ctx, ID) {
        try {
            if (ctx.mspID === Utils.externalActorsMSP) {
                return shim.error(`Function forbidden for ${Utils.externalActorsMSP}.`)
            }
            ...
            let message = `Error in getContractById function: ${e}`
            Utils.dumpError(e);
            return shim.error(message)
        }
    }
     ...
     *
     *
     * @param {String} userIDCaller the ID of user calling function
     *
     * @param {String} userIDQuery the ID of user query
     *
     ...
     * @throws {shim.error} transaction execution failure
     */
    async checkUsersEnrolledInAcceptedOffer(ctx, userIDCaller,userIDQuery, queryUserType) {
        try {

            let jsonRes = {};
            jsonRes.connected = false;
            let buyer = false;
            let seller = false;
            ...
            let acceptedOffers = await this.queryWithQueryString(ctx, JSON.stringify(queryString));

            for(let offer of acceptedOffers) {
                let buyers = Object.keys(offer.Record.acceptedBuyers)
                if (queryUserType === 'buyer'){
                    if(buyers.includes(userIDQuery)) {
                        buyer = true;
                    }
                    if (offer.Record.owner === userIDCaller){
                        seller = true;
                    }
                }
                else {
                    if(buyers.includes(userIDCaller)) {
                        buyer = true;
                    }
                    if (offer.Record.owner === userIDQuery){
                        seller = true;
                    }
                }

                if (buyer && seller){
                    jsonRes.connected = true;
                    return jsonRes;
                }
            ...
            let message = `Error in checkUsersEnrolledInAcceptedOffer function: ${e}`
            Utils.dumpError(e);
            return shim.error(message)
        }
    }
    ...
    /**
     * Getting direct offer for specific offer ID.
     *
     * @param {Context} ctx the transaction context
     * @param {String} ID the ID of offer
     *
     * @returns {JSON} the query results
     * @throws {shim.error} transaction execution failure
     */
    async getOfferById(ctx, ID) {
        try {
            if (!(ctx.mspID === Utils.mpoMSP || ctx.mspID === Utils.retailerMSP || ctx.mspID === Utils.dsoMSP)) {
                return shim.error(`Function private to ${Utils.mpoMSP}, ${Utils.retailerMSP} or ${Utils.dsoMSP}.`)
            }
            ...
            let message = `Error in getOfferById function: ${e}`
            Utils.dumpError(e);
            return shim.error(message)
        }
    }
...
module.exports = DirectContractsContract